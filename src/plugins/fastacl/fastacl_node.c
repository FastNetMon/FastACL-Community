...
format_fastacl_trace (u8 *s, va_list *args)
{
  ...
  return format (s, "fastacl: rule %u (order %u) action %s next %u pkts %llu",
		 t->rule_index, t->rule_order,
		 fastacl_action_type_name (t->action_type), t->next_index,
		 t->pkt_count);
}
#endif
...
fastacl_sample_psample (fastacl_main_t *fsm, vlib_buffer_t *b0, u32 pkt_bytes,
			u32 rule_index, const fastacl_rule_t *rule,
			fastacl_per_worker_t *per_worker, u32 *n_sampled,
			u32 *n_sample_fail)
{
  ...
  if (PREDICT_FALSE (fastacl_psample_send (&per_worker->psample,
					   rule->action.sample_group, ratio,
...
fastacl_apply_rule (fastacl_main_t *fsm, u32 rule_index, u8 action_type,
		    int is_traced, vlib_main_t *vm, u32 pkt_bytes,
		    ip4_header_t *pkt_ip, int is_ip6, u32 *n_matched,
		    u32 *n_dropped, u32 *trace_rule_index,
		    u32 *trace_rule_order, u8 *trace_action_type,
		    fastacl_per_worker_t *per_worker)
{

  if (PREDICT_TRUE (fsm->rule_stats_enabled) &&
      PREDICT_TRUE (rule_index < fsm->rule_counter_len))
    vlib_increment_combined_counter (&fsm->rule_counters, vm->thread_index,
				     rule_index, 1, pkt_bytes);
  (*n_matched)++;
...
static_always_inline void
fastacl_record_trace (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_buffer_t *b0, fastacl_main_t *fsm, u32 rule_index,
		      u32 rule_order, u8 action_type, u32 next_index)
{
  ...
  t->pkt_count = (rule_index != ~0u) ?
		   pool_elt_at_index (fsm->rules, rule_index)->packet_count :
		   0;
}
...
fastacl_parse_ip4 (fastacl_parsed_t *p, ip4_header_t *ip4, u16 avail)
{
  ...
  int df = !!(ip4->flags_and_fragment_offset &
  ...
  p->l4 = (frag_off == 0 && ip_bytes >= sizeof (ip4_header_t) &&
	   ip_bytes < p->pkt_len) ?
	    (u8 *) ip4 + ip_bytes :
	    NULL;
}
...
static_always_inline void
fastacl_parse_ip6 (fastacl_parsed_t *p, vlib_buffer_t *b, ip6_header_t *ip6,
		   u16 avail)
{
  ip6_ext_hdr_chain_t chain;
  u16 claimed =
    clib_net_to_host_u16 (ip6->payload_length) + (u16) sizeof (ip6_header_t);
  ...
  int last =
    ip6_ext_header_walk (b, ip6, IP_PROTOCOL_IPV6_FRAGMENTATION, &chain);
  if (last < 0)
    return;
  ...
  if (chain.eh[last].protocol == IP_PROTOCOL_IPV6_FRAGMENTATION)
    {
      ip6_frag_hdr_t *fh =
	(ip6_frag_hdr_t *) ((u8 *) ip6 + chain.eh[last].offset);
      u16 frag_off = ip6_frag_hdr_offset (fh);

      p->pkt_frag_flags =
	fastacl_compute_frag_flags (ip6_frag_hdr_more (fh), frag_off);
...
{
  ip4_header_t *ip0 =
    (ip4_header_t *) ((u8 *) vlib_buffer_get_current (b) + l2_ofs);
  u8 version = (ip0->ip_version_and_header_length >> 4);
  u16 avail =
    (b->current_length > l2_ofs) ? (u16) (b->current_length - l2_ofs) : 0;
  ...
  p->l4_bytes =
    p->l4 ? (u16) (p->pkt_len - (u16) ((u8 *) p->l4 - (u8 *) p->ip0)) : 0;
...
fastacl_classify_one (fastacl_main_t *fsm, const fastacl_parsed_t *p,
		      u8 *action_out)
{
  if (PREDICT_FALSE (p->is_ip6))
    {
      ip6_header_t *ip6 = (ip6_header_t *) p->ip0;
      return fastacl_tss_lookup_ip6 (
	fsm, ip6, p->pkt_proto, p->l4_src_port, p->l4_dst_port, p->l4,
	p->l4_bytes, p->pkt_len, p->pkt_dscp, p->pkt_frag_flags, action_out);
    }

  return fastacl_tss_lookup_ip4 (
    fsm, p->ip0, p->pkt_proto, p->l4_src_port, p->l4_dst_port, p->l4,
    p->l4_bytes, p->pkt_len, p->pkt_dscp, p->pkt_frag_flags, action_out);
}

static_always_inline uword
fastacl_node_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_frame_t *frame, int is_l2)
{
  ...
  if (n_left > 0)
    fastacl_parse_packet (b[0], &p_cur,
			  is_l2 ? vnet_buffer (b[0])->l2.l2_len : 0);
  ...
  while (n_left > 0)
	...
	{
	  fastacl_parse_packet (b[1], &p_next,
				is_l2 ? vnet_buffer (b[1])->l2.l2_len : 0);
	  if (PREDICT_TRUE (!p_next.unparseable))
	    {
	      if (PREDICT_FALSE (p_next.is_ip6))
		fastacl_tss_prefetch_ip6 (fsm, (ip6_header_t *) p_next.ip0,
					  p_next.pkt_proto);
	      else
		fastacl_tss_prefetch_ip4 (fsm, p_next.ip0, p_next.pkt_proto,
					  p_next.pkt_len);
	    }
	}
      ...
      else
	{
	  ...
	  if (ri != ~0u)
	      ...
	      if (PREDICT_FALSE (fsm->n_sample_rules != 0))
		{
		  ...
		  fastacl_sample_psample (fsm, b[0], b0_bytes, ri, sr,
					  per_worker, &n_sampled,
					  &n_sample_fail);
		}
	      if (fastacl_apply_rule (fsm, ri, action_type, is_traced, vm,
				      b0_bytes, p_cur.ip0, p_cur.is_ip6,
				      &n_matched, &n_dropped, &trace_ri,
				      &trace_order, &trace_action, per_worker))
		{
	...
	fastacl_record_trace (vm, node, b[0], fsm, trace_ri, trace_order,
			      trace_action, next0);
  ...
  vlib_node_increment_counter (vm, node->node_index, FASTACL_ERROR_MATCHED,
			       n_matched);
  vlib_node_increment_counter (vm, node->node_index, FASTACL_ERROR_PASSED,
			       n_passed);
  vlib_node_increment_counter (vm, node->node_index, FASTACL_ERROR_UNPARSEABLE,
			       n_unparseable);
  vlib_node_increment_counter (vm, node->node_index, FASTACL_ERROR_SAMPLED,
			       n_sampled);
  vlib_node_increment_counter (vm, node->node_index, FASTACL_ERROR_SAMPLE_FAIL,
			       n_sample_fail);
