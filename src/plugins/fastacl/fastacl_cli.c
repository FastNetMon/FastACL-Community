...
static clib_error_t *
fastacl_cli_parse_action (unformat_input_t *input, fastacl_action_t *action,
			  int *have_action, int *handled)
{
  u32 dscp;
...
static clib_error_t *
fastacl_cli_match_prefix (fastacl_match_t *m, u32 flag, int is_ip6,
			  const void *addr, u32 plen)
{
  int is_dst = flag == FASTACL_MATCH_DST_PREFIX;
  ip46_address_t *dst = is_dst ? &m->dst_addr : &m->src_addr;
  clib_error_t *e = fastacl_cli_plen (
    plen, is_ip6, is_dst ? &m->dst_prefix_len : &m->src_prefix_len);
...
static clib_error_t *
fastacl_rule_add_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	e = fastacl_cli_match_prefix (&match, FASTACL_MATCH_DST_PREFIX, 0,
				      &addr4, plen);
      else if (unformat (input, "dst6 %U/%u", unformat_ip6_address, &addr6,
			 &plen))
	e = fastacl_cli_match_prefix (&match, FASTACL_MATCH_DST_PREFIX, 1,
				      &addr6, plen);
      else if (unformat (input, "src %U/%u", unformat_ip4_address, &addr4,
			 &plen))
	e = fastacl_cli_match_prefix (&match, FASTACL_MATCH_SRC_PREFIX, 0,
				      &addr4, plen);
      else if (unformat (input, "src6 %U/%u", unformat_ip6_address, &addr6,
			 &plen))
	e = fastacl_cli_match_prefix (&match, FASTACL_MATCH_SRC_PREFIX, 1,
				      &addr6, plen);
      else if (unformat (input, "proto %u", &val))
	{
      ...
      else if (unformat (input, "dst-port %U", unformat_fastacl_range, &lo,
			 &hi))
	{
      ...
      else if (unformat (input, "src-port %U", unformat_fastacl_range, &lo,
			 &hi))
	{
      ...
      else if (unformat (input, "either-port %U", unformat_fastacl_range, &lo,
			 &hi))
	{
      ...
      else if (unformat (input, "icmp-code %u", &val))
	{
      ...
      else if (unformat (input, "tcp-flags value %U mask %U",
			 unformat_vlib_number, &val, unformat_vlib_number,
			 &mask))
	{
      ...
      else if (unformat (input, "pkt-len %U", unformat_fastacl_range, &lo,
			 &hi))
	{
      ...
      else if (unformat (input, "dscp %u", &val))
	{
      ...
      else if (unformat (input, "fragment flags %U mask %U",
			 unformat_vlib_number, &val, unformat_vlib_number,
			 &mask))
	{
	...
	{
	  e =
	    fastacl_cli_parse_action (input, &action, &have_action, &handled);
	  if (!e && !handled)
  ...
  if (!have_action)
    return clib_error_return (
      0, "an action is required: drop or permit (pass/accept).  Refusing to "
...
VLIB_CLI_COMMAND (fastacl_rule_add_command, static) = {
  ...
  .short_help =
    "fastacl rule add order <N> [dst <ip4>/<len>] [dst6 <ip6>/<len>] "
...
static clib_error_t *
fastacl_rule_del_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
...
static clib_error_t *
fastacl_rule_stats_command_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "per-rule stats: %s",
		   fsm->rule_stats_enabled ? "enabled" : "disabled");
  return 0;
}
...
static clib_error_t *
fastacl_show_tuples_command_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  fastacl_main_t *fsm = &fastacl_main;
  fastacl_tuple_t *t;
  ...
  vlib_cli_output (vm, "%-6s %-10s %-6s %-6s %-6s %-6s %-8s %-8s %-8s %-8s",
		   "id", "flags", "dplen", "splen", "v6", "key", "rules",
		   "chains", "chained", "deepest");
  pool_foreach (t, fsm->tuples)
    {
      ...
      vlib_cli_output (vm,
		       "%-6u 0x%-8x %-6u %-6u %-6u %-6s %-8u %-8u %-8u %-8u",
		       (u32) (t - fsm->tuples), t->mask.flags,
		       t->mask.dst_plen, t->mask.src_plen, t->mask.is_ip6,
		       keyw, t->n_rules, n_chains, chained, deepest);
    }
  return 0;
}
...
fastacl_show_rules_flat (vlib_main_t *vm, fastacl_main_t *fsm, u32 *sorted)
{
  ...
  vlib_cli_output (
    vm, "%-8s %-8s %-20s %-20s %-8s %-32s %-12s %-12s %-12s %-12s %-14s %-14s",
    "Index", "Order", "Dst Prefix", "Src Prefix", "Proto", "Match Extra",
    "Action", "Packets", "Bytes", "pps", "L3 bps", "L1 bps");
  for (i = 0; i < vec_len (sorted); i++)
    fastacl_show_one_rule (vm, pool_elt_at_index (fsm->rules, sorted[i]));
}

static clib_error_t *
fastacl_show_rules_command_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  fastacl_main_t *fsm = &fastacl_main;
...
static clib_error_t *
fastacl_set_interface_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
  ...
  if (rv)
    return clib_error_return (
      0, "fastacl_interface_enable_disable returned %d", rv);
...
static clib_error_t *
fastacl_show_aggregate_counters_command_fn (vlib_main_t *vm,
					    ...
					    vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "  %-12s %-22s %-22s %-14s %-14s %-14s", "", "Packets",
		   "Bytes", "pps", "L3 bps", "L1 bps");
  vlib_cli_output (
    vm, "  %-12s %llu (%U)  %llu (%U)  %U  %U  %U",
    "Processed:", fsm->counters.total_processed, format_fastacl_count_si,
    fsm->counters.total_processed, fsm->counters.total_bytes_processed,
    format_fastacl_bytes_si, fsm->counters.total_bytes_processed,
    format_fastacl_pps, fsm->counters.processed_rate.pps, format_fastacl_bps,
    fsm->counters.processed_rate.l3_bps, format_fastacl_bps,
    fsm->counters.processed_rate.l1_bps);
  vlib_cli_output (vm, "  %-12s %llu (%U)  %llu (%U)  %U  %U  %U",
		   "Dropped:", fsm->counters.total_dropped,
		   format_fastacl_count_si, fsm->counters.total_dropped,
		   fsm->counters.total_bytes_dropped, format_fastacl_bytes_si,
		   fsm->counters.total_bytes_dropped, format_fastacl_pps,
		   fsm->counters.dropped_rate.pps, format_fastacl_bps,
		   fsm->counters.dropped_rate.l3_bps, format_fastacl_bps,
		   fsm->counters.dropped_rate.l1_bps);
  return 0;
}
...
static clib_error_t *
fastacl_psample_command_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  ...
  if (rv == VNET_API_ERROR_FEATURE_DISABLED)
    return clib_error_return (
      0, "psample netlink family not found - is the psample module loaded?");
  if (rv)
...
static clib_error_t *
fastacl_show_sampling_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
  fastacl_main_t *fsm = &fastacl_main;
  fastacl_rule_t *rule;

  vlib_cli_output (vm, "psample: %s",
		   fsm->psample_enabled ? "enabled" : "disabled");
  if (fsm->psample.family_id)
    vlib_cli_output (vm, "psample multicast group id: %u",
		     fsm->psample.group_id);
  ...
  vlib_cli_output (vm, "%-8s %-8s %-8s %-14s %-14s", "rule", "ratio", "group",
		   "sampled", "send-failed");
  ...
  pool_foreach (rule, fsm->rules)
      ...
      vec_foreach (pw, fsm->per_worker)
	{
      ...
      vlib_cli_output (vm, "%-8u 1:%-6u %-8u %-14llu %-14llu", rule->index,
		       rule->action.sample_ratio, rule->action.sample_group,
		       taken, missed);
    }
  return 0;
}
