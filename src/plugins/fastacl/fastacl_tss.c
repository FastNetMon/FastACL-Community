...
static_always_inline void
key_from_fields_ip4 (u32 flags, u32 dst, u8 dst_plen, u32 src, u8 src_plen,
		     u8 proto, clib_bihash_kv_24_8_t *kv)
{
  tss_key_pack (kv,
		masked_ip4 (flags, FASTACL_MATCH_SRC_PREFIX, src, src_plen),
...
{
  key_from_fields_ip4 (m->flags, m->dst_addr.ip4.as_u32, m->dst_prefix_len,
		       m->src_addr.ip4.as_u32, m->src_prefix_len, m->proto,
		       kv);
}

static_always_inline void
key_from_packet_ip4 (const fastacl_tuple_mask_t *mask, ip4_header_t *ip,
		     u8 pkt_proto, clib_bihash_kv_24_8_t *kv)
{
  key_from_fields_ip4 (mask->flags, ip->dst_address.as_u32, mask->dst_plen,
		       ip->src_address.as_u32, mask->src_plen, pkt_proto, kv);
}

static_always_inline void
tss6_key_pack (clib_bihash_kv_40_8_t *kv, u64 da0, u64 da1, u64 sa0, u64 sa1,
	       u64 proto)
{
  kv->key[0] = da0;
  kv->key[1] = da1;
...
static_always_inline void
key_from_fields_ip6 (u32 flags, const ip6_address_t *dst, u8 dst_plen,
		     const ip6_address_t *src, u8 src_plen, u8 proto,
		     clib_bihash_kv_40_8_t *kv)
{
  ...
  tss6_key_pack (kv, da0, da1, sa0, sa1,
		 (flags & FASTACL_MATCH_PROTO) ? proto : 0);
}

static_always_inline void
key_from_rule_ip6 (const fastacl_match_t *m, clib_bihash_kv_40_8_t *kv)
{
  key_from_fields_ip6 (m->flags, &m->dst_addr.ip6, m->dst_prefix_len,
		       &m->src_addr.ip6, m->src_prefix_len, m->proto, kv);
}

static_always_inline void
key_from_packet_ip6 (const fastacl_tuple_mask_t *mask, ip6_header_t *ip6,
		     u8 pkt_proto, clib_bihash_kv_40_8_t *kv)
{
  key_from_fields_ip6 (mask->flags, &ip6->dst_address, mask->dst_plen,
		       &ip6->src_address, mask->src_plen, pkt_proto, kv);
}

static_always_inline int
tss_full_rule_match_ip6 (fastacl_match_t *m, ip6_header_t *ip6, u8 pkt_proto,
			 u16 l4_src_port, u16 l4_dst_port, u8 *l4,
			 u16 l4_bytes, u16 pkt_len, u8 pkt_dscp,
			 u8 pkt_frag_flags)
{

  if (!(m->flags & FASTACL_MATCH_IS_IP6) &&
      (m->flags & FASTACL_MATCH_ADDR_BITS))
    return 0;
  if ((m->flags & FASTACL_MATCH_DST_PREFIX) &&
      !fastacl_ip6_prefix_match (m->flags, FASTACL_MATCH_DST_PREFIX,
				 &ip6->dst_address, &m->dst_addr.ip6,
				 m->dst_prefix_len))
    return 0;
  if ((m->flags & FASTACL_MATCH_SRC_PREFIX) &&
      !fastacl_ip6_prefix_match (m->flags, FASTACL_MATCH_SRC_PREFIX,
				 &ip6->src_address, &m->src_addr.ip6,
				 m->src_prefix_len))
    return 0;
  return fastacl_match_fields (m, pkt_proto, l4_src_port, l4_dst_port, l4,
...
mask_is_compact (const fastacl_tuple_mask_t *mask)
{
  ...
  return !(mask->flags & FASTACL_MATCH_SRC_PREFIX) ||
	 !(mask->flags & FASTACL_MATCH_PROTO);
}
...
{
  clib_bihash_kv_8_8_t k = { .key = kv->key[0] | (kv->key[1] << 32),
			     .value = value };
  return k;
}
...
{
  clib_bihash_kv_16_8_t k = { .key = { kv->key[0], kv->key[1] },
			      .value = value };
  return k;
}
...
{
  const u32 keyed = FASTACL_MATCH_DST_PREFIX | FASTACL_MATCH_SRC_PREFIX |
		    FASTACL_MATCH_PROTO | FASTACL_MATCH_IS_IP6;
...
fastacl_tss_clear (fastacl_main_t *fsm)
{
  ...
  fsm->tuple_by_mask =
    hash_create_mem (0, sizeof (fastacl_tuple_mask_t), sizeof (uword));
}
...
tss_chain_for_insert (fastacl_main_t *fsm, fastacl_tuple_t *t, u64 value,
		      u32 *new_chain_index)
{
  ...
  *new_chain_index = chain - fsm->chains;
  ...
  sorted_vec_insert (fsm, &chain->rule_indices,
		     fastacl_val_rule_index (value));
  return chain;
}
...
tss_rule_value (fastacl_main_t *fsm, u32 rule_index)
{
  ...
  return fastacl_val_make_rule (rule_index, rule_key_complete (&rule->match),
				rule->action.type);
}

static fastacl_chain_outcome_t
tss_chain_left (fastacl_main_t *fsm, fastacl_tuple_t *t, u64 value,
		u32 rule_index, u64 *replacement)
{
...
static_always_inline void
tss_key_from_rule (const fastacl_tuple_t *t, const fastacl_match_t *m,
...
tss_bihash_insert (fastacl_main_t *fsm, fastacl_tuple_t *t, u32 rule_index)
{
  ...
  fastacl_chain_t *chain =
    tss_chain_for_insert (fsm, t, value, &new_chain_index);
...
static int
tss_value_holds_duplicate (fastacl_main_t *fsm, u64 value, u32 order,
			   const fastacl_match_t *match)
{
  ...
  if (fastacl_val_is_chain (value))
    {
      fastacl_chain_t *chain =
	pool_elt_at_index (fsm->chains, fastacl_val_chain_index (value));
  ...
  for (u32 i = 0; i < n_candidates; i++)
    {
      ...
      if (rule->order == order &&
	  clib_memcmp (&rule->match, match, sizeof (*match)) == 0)
	return 1;
    }
...
int
fastacl_tss_rule_exists (fastacl_main_t *fsm, u32 order,
			 const fastacl_match_t *match)
{
...
static void
tss_insert_one (fastacl_main_t *fsm, u32 rule_index,
		const fastacl_tuple_mask_t *mask)
{
...
static void
tss_remove_one (fastacl_main_t *fsm, u32 rule_index,
		const fastacl_tuple_mask_t *mask)
{
  uword *p = hash_get_mem (fsm->tuple_by_mask, mask);
  ...
  fastacl_u32_vec_remove_value (tuple_family_vec (fsm, t->mask.is_ip6),
				t - fsm->tuples);
  hash_unset_mem_free (&fsm->tuple_by_mask, &t->mask);
  tuple_free_resources (t);
...
void
fastacl_tuple_chain_stats (fastacl_main_t *fsm, const fastacl_tuple_t *t,
			   u32 *n_chains, u32 *max_depth, u32 *n_chained_rules)
{
...
static_always_inline int
tss_full_rule_match_ip4 (fastacl_match_t *m, ip4_header_t *ip0, u8 pkt_proto,
			 u16 l4_src_port, u16 l4_dst_port, u8 *l4,
			 u16 l4_bytes, u16 pkt_len, u8 pkt_dscp,
			 u8 pkt_frag_flags)
{
  if (m->flags & FASTACL_MATCH_IS_IP6)
    return 0;
  if (!fastacl_ip4_prefix_match (m->flags, FASTACL_MATCH_DST_PREFIX,
				 &ip0->dst_address, &m->dst_addr.ip4,
				 m->dst_prefix_len))
    return 0;
  if (!fastacl_ip4_prefix_match (m->flags, FASTACL_MATCH_SRC_PREFIX,
				 &ip0->src_address, &m->src_addr.ip4,
				 m->src_prefix_len))
    return 0;
  return fastacl_match_fields (m, pkt_proto, l4_src_port, l4_dst_port, l4,
			       l4_bytes, pkt_len, pkt_dscp, pkt_frag_flags);
...
static_always_inline int
tss_candidate_matches (int is_ip6, fastacl_match_t *m, ip4_header_t *ip,
		       ip6_header_t *ip6, u8 pkt_proto, u16 l4_src_port,
		       u16 l4_dst_port, u8 *l4, u16 l4_bytes, u16 pkt_len,
		       u8 pkt_dscp, u8 pkt_frag_flags)
{
  if (is_ip6)
    return tss_full_rule_match_ip6 (m, ip6, pkt_proto, l4_src_port,
				    l4_dst_port, l4, l4_bytes, pkt_len,
				    pkt_dscp, pkt_frag_flags);
  return tss_full_rule_match_ip4 (m, ip, pkt_proto, l4_src_port, l4_dst_port,
				  l4, l4_bytes, pkt_len, pkt_dscp,
				  pkt_frag_flags);
}

static_always_inline void
tss_consider (fastacl_main_t *fsm, u64 result_value, fastacl_tss_best_t *best,
	      int is_ip6, ip4_header_t *ip, ip6_header_t *ip6, u8 pkt_proto,
	      u16 l4_src_port, u16 l4_dst_port, u8 *l4, u16 l4_bytes,
	      u16 pkt_len, u8 pkt_dscp, u8 pkt_frag_flags)
{
  if (PREDICT_TRUE (!fastacl_val_is_chain (result_value)))
	...
	return;
	  ...
	  tss_candidate_matches (is_ip6, &rule->match, ip, ip6, pkt_proto,
				 l4_src_port, l4_dst_port, l4, l4_bytes,
				 pkt_len, pkt_dscp, pkt_frag_flags))
	fastacl_tss_best_take (best, ri, rule,
			       fastacl_val_action (result_value));
      return;
    }

  fastacl_chain_t *chain =
    pool_elt_at_index (fsm->chains, fastacl_val_chain_index (result_value));
  ...
  for (u32 i = 0; i < n; i++)
    {
      ...
      if (!fastacl_rule_outranks (rule, ri, best->rule, best->ri))
	break;
      if (tss_candidate_matches (is_ip6, &rule->match, ip, ip6, pkt_proto,
				 l4_src_port, l4_dst_port, l4, l4_bytes,
				 pkt_len, pkt_dscp, pkt_frag_flags))
	{
...
u32
fastacl_tss_lookup_ip4 (fastacl_main_t *fsm, ip4_header_t *ip, u8 pkt_proto,
			u16 l4_src_port, u16 l4_dst_port, u8 *l4, u16 l4_bytes,
			u16 pkt_len, u8 pkt_dscp, u8 pkt_frag_flags,
			u8 *action_out)
{
  ...
  vec_foreach (ti, fsm->tuple_index_v4)
	...
	continue;
      ...
      tss_consider (fsm, result_value, &best, 0, ip, 0, pkt_proto, l4_src_port,
		    l4_dst_port, l4, l4_bytes, pkt_len, pkt_dscp,
		    pkt_frag_flags);
    }
...
void
fastacl_tss_prefetch_ip4 (fastacl_main_t *fsm, ip4_header_t *ip, u8 pkt_proto,
			  u16 pkt_len)
{
  u32 *ti;
...
u32
fastacl_tss_lookup_ip6 (fastacl_main_t *fsm, ip6_header_t *ip6, u8 pkt_proto,
			u16 l4_src_port, u16 l4_dst_port, u8 *l4, u16 l4_bytes,
			u16 pkt_len, u8 pkt_dscp, u8 pkt_frag_flags,
			u8 *action_out)
{
  ...
  vec_foreach (ti, fsm->tuple_index_v6)
	...
	continue;
      ...
      tss_consider (fsm, result_value, &best, 1, 0, ip6, pkt_proto,
		    l4_src_port, l4_dst_port, l4, l4_bytes, pkt_len, pkt_dscp,
		    pkt_frag_flags);
    }
  *action_out = best.action;
  return best.ri;
